...
func (stream *TCPStream) addPacket(pkt *protos.Packet, tcphdr *layers.TCP) {
	...
	if len(pkt.Payload) > 0 {
		...
		tlsMap := mod.GetMap()
		if len(tlsMap) > 0 {
			// fmt.Println("server name before clearing name", tlsMap["server"])
			mod.InitMap()
			stream.tlsMap = tlsMap
			// fmt.Println("server name after clearing name", stream.tlsMap["server"])
			// fmt.Println("#####################################################")
		}
	}
...
func (tcp *TCP) Process(id *flows.FlowID, tcphdr *layers.TCP, pkt *protos.Packet) {
	...
	if len(stream.tlsMap) > 0 {
		tcp.TlsMap = stream.tlsMap
	}
}
